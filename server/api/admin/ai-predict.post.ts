...
export default defineEventHandler(async (event) => {
  ...
  const body = await readBody<{ raceId: string, leagueId: string }>(event)
  ...
  if (!league.pitwallEnabled) throw createError({ statusCode: 403, message: 'Pitwall is not enabled for this league.' })
  ...
  if (!botUser) throw createError({ statusCode: 500, message: 'Failed to create Pitwall user' })
  ...
  const [existing] = await db.select().from(schema.prediction)
    .where(and(
      ...
      eq(schema.prediction.leagueId, body.leagueId),
    ))
    .limit(1)
