  ...
  <div class="flex items-center justify-center min-h-[calc(100dvh-60px)] px-4">
    <div class="w-full max-w-sm">
      ...
      <div class="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6">
        <UForm v-if="step === 'info'" :schema="infoSchema" :state="infoState" class="flex flex-col gap-4" @submit="sendCode" @keydown.meta.enter.prevent="($event.target as HTMLElement).closest('form')?.requestSubmit()">
        ...
        <UForm v-else :schema="otpSchema" :state="otpState" class="flex flex-col gap-4" @submit="verifyAndRegister" @keydown.meta.enter.prevent="($event.target as HTMLElement).closest('form')?.requestSubmit()">
