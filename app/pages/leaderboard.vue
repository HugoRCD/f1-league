...
<template>
  <UContainer class="py-8">
    ...
    <template v-else>
      <div v-if="leaderboard.length >= 3" class="flex items-end justify-center gap-3 mb-10">
        <div class="w-40 text-center">
          <div class="flex justify-center mb-3">
            <div class="ring-2 ring-zinc-400/40 rounded-full">
              <UserAvatar :image="leaderboard[1].userImage" :name="leaderboard[1].userName" size="md" />
            </div>
          </div>
          <p class="font-bold text-sm truncate">{{ leaderboard[1].userName }}</p>
          <div class="mt-3 rounded-t-xl bg-zinc-400/10 border border-zinc-400/20 border-b-0 pt-4 pb-6 px-3">
            <p class="text-3xl font-black tabular-nums text-zinc-300">{{ leaderboard[1].totalPoints }}</p>
            <p class="text-[10px] text-zinc-500 mt-1">{{ leaderboard[1].raceWins }}W · {{ leaderboard[1].totalExactHits }}E</p>
          </div>
        ...
        <div class="w-44 text-center -mb-2">
          ...
          <div class="flex justify-center mb-3">
            <div class="ring-3 ring-yellow-500/40 rounded-full">
              <UserAvatar :image="leaderboard[0].userImage" :name="leaderboard[0].userName" size="lg" />
            </div>
          </div>
          <p class="font-bold truncate">{{ leaderboard[0].userName }}</p>
          <div class="mt-3 rounded-t-xl bg-yellow-500/10 border border-yellow-500/20 border-b-0 pt-6 pb-8 px-3">
            <p class="text-4xl font-black tabular-nums text-yellow-400">{{ leaderboard[0].totalPoints }}</p>
            <p class="text-[10px] text-zinc-500 mt-1">{{ leaderboard[0].raceWins }}W · {{ leaderboard[0].totalExactHits }}E</p>
          </div>
        ...
        <div class="w-36 text-center">
          <div class="flex justify-center mb-3">
            <div class="ring-2 ring-amber-700/40 rounded-full">
              <UserAvatar :image="leaderboard[2].userImage" :name="leaderboard[2].userName" size="md" />
            </div>
          </div>
          <p class="font-bold text-sm truncate">{{ leaderboard[2].userName }}</p>
          <div class="mt-3 rounded-t-xl bg-amber-700/10 border border-amber-700/20 border-b-0 pt-3 pb-4 px-3">
            <p class="text-2xl font-black tabular-nums text-amber-500">{{ leaderboard[2].totalPoints }}</p>
            <p class="text-[10px] text-zinc-500 mt-1">{{ leaderboard[2].raceWins }}W · {{ leaderboard[2].totalExactHits }}E</p>
          </div>
